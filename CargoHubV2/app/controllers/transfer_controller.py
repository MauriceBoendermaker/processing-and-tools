...
from CargoHubV2.app.services import transfer_service
...
def create_transfer(transfer: transfers_schema.TransferCreate, db: Session = Depends(get_db)):
    db_transfer = transfer_service.create_transfer(db, transfer)
...
def get_transfers(db: Session = Depends(get_db), id: Optional[int] = None):
    if id:
        transfer = transfer_service.get_transfer(db, id)
        if transfer is None:
            raise HTTPException(status_code=404, detail="Transfer not found")
        return transfer
    transfers = transfer_service.get_all_transfers(db)
    return transfers


@router.delete("/{id}")
def delete_transfer(id: int, db: Session = Depends(get_db)):
    success = transfer_service.delete_transfer(db, id)
    if success:
...
def update_transfer(id: int, transfer_data: transfers_schema.TransferUpdate, db: Session = Depends(get_db)):
    updated_transfer = transfer_service.update_transfer(db, id, transfer_data)
    if updated_transfer:
