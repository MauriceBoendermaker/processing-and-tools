...
def create_item_endpoint(item_data: ItemCreate, db: Session = Depends(get_db)):
    item = create_item(db, item_data.dict())
    return item
...
def update_item_endpoint(uid: str, item_data: ItemCreate, db: Session = Depends(get_db)):
    item = update_item(db, uid, item_data.dict())
    if not item:
        raise HTTPException(status_code=404, detail="Item not found")
    return item
