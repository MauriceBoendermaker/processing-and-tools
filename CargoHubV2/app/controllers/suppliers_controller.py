...
@router.get("/")
def get_suppliers(code: Optional[str] = None, offset: int = 0, limit: int = 100, db: Session = Depends(get_db), api_key: str = Header(...),):
    if code:
...
def update_supplier_endpoint(code: str, supplier_data: SuppliersUpdate, db: Session = Depends(get_db), api_key: str = Header(...)):
