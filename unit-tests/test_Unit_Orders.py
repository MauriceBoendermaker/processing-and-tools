...
def test_create_order_integrity_error():
    ...
    assert "An order with this data already exists." in str(excinfo.value.detail)
...
def test_update_order_integrity_error():
    ...
    assert excinfo.value.status_code == 400
    assert "An integrity error occurred while updating the order." in str(excinfo.value.detail)
    db.rollback.assert_called_once()
