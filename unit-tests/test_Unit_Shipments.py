...
def test_create_shipment_integrity_error():
    db = MagicMock()
    ...
    with pytest.raises(HTTPException) as excinfo:
        create_shipment(db, shipment_data.model_dump())

    assert excinfo.value.status_code == 400
    assert "A shipment with this ID already exists." in str(excinfo.value.detail)
    db.rollback.assert_called_once()  # Ensure rollback is called


def test_get_shipment_found():
    db = MagicMock()
...
def test_delete_shipment_not_found():
    db = MagicMock()
    db.query().filter().first.return_value = None

    with pytest.raises(HTTPException) as excinfo:
        delete_shipment(db, 2)
