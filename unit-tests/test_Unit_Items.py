...
def test_create_item_integrity_error():
    ...
    assert "An item with this code already exists." in str(excinfo.value.detail)
...
def test_update_item_integrity_error():
    ...
    assert excinfo.value.status_code == 400
    assert "An integrity error occurred while updating the item." in str(excinfo.value.detail)
    db.rollback.assert_called_once()
