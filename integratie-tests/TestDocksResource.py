...
class TestDocksResource(unittest.TestCase):
    ...
    def test_1_post_dock(self):
        ...
        # Store the created_id at instance level
        self.created_id = response.json().get("id")
    ...
    def test_4_get_dock_by_id(self):
        self.assertIsNotNone(self.created_id, "created_id should not be None. Did test_1_post_dock run first?")
        response = self.client.get(f"{self.baseUrl}{self.created_id}")
        self.assertEqual(response.status_code, 200)
        body = response.json()
        self.assertEqual(body.get("id"), self.created_id)
        self.assertEqual(body.get("code"), self.TEST_BODY["code"])
        self.assertEqual(body.get("description"), self.TEST_BODY["description"])

    def test_5_put_dock(self):
        self.assertIsNotNone(self.created_id, "created_id should not be None. Did test_1_post_dock run first?")
        response = self.client.put(f"{self.baseUrl}{self.created_id}", json=self.ToPut)
        self.assertEqual(response.status_code, 200)
    ...
    def test_6_delete_dock(self):
        self.assertIsNotNone(self.created_id, "created_id should not be None.")
        response = self.client.delete(f"{self.baseUrl}{self.created_id}")
        self.assertEqual(response.status_code, 200)
